...
# Create category
@router.post("/categories", response_model=CategorySchema, status_code=201, dependencies=[Depends(JWTBearer())])
...
# Get all category
@router.get("/categories", response_model=List[CategorySchema], dependencies=[Depends(JWTBearer())])
...
# Get category by ID
@router.get("/categories/{category_id}", response_model=CategorySchema, dependencies=[Depends(JWTBearer())])
...
# Update a category
@router.put("/categories/{category_id}", response_model=CategorySchema, dependencies=[Depends(JWTBearer())])
...
# Soft delete
@router.delete("/categories/{category_id}", response_model=CategorySchema, dependencies=[Depends(JWTBearer())])
...
# Restore Soft delete
@router.post("/categories/{category_id}", dependencies=[Depends(JWTBearer())])
