...
from api.utils.jwt_util import get_current_user
...
async def update_existing_post(post_id: int, post_update: PostUpdate, db: AsyncSession = Depends(async_get_db), current_user: TokenData = Depends(get_current_user)):
    return await update_post(db, post_id, post_update, current_user)

# Create a new post
@router.post("/posts", response_model=Post, status_code=201)
async def create_new_post(post: PostCreate, db: AsyncSession = Depends(async_get_db), current_user: TokenData = Depends(get_current_user)):

    db_post = await create_post(db, post, current_user)
    return db_post
